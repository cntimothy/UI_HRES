...
namespace HRES
{
    public class PageBase : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// 根据文档当前状态和操作获取对文档进行操作后的文档状态
        /// </summary>
        /// <param name="currentStatus">当前状态</param>
        /// <param name="operation">操作</param>
        /// <returns>文档的下一个状态</returns>
        protected DocStatus GetNextDocStatus(DocStatus currentStatus, DocOperation operation)
        {
            DocStatus nextStatus = currentStatus;
            if (currentStatus == DocStatus.unmake)
            {
                switch (operation)
                {
                    case DocOperation.save:
                        nextStatus = DocStatus.saved;
                        break;
                    case DocOperation.submit:
                        nextStatus = DocStatus.submitted;
                        break;
                    case DocOperation.pass:
                        break;
                    case DocOperation.reject:
                        break;
                    default:
                        break;
                }
            ...
            else if (currentStatus == DocStatus.submitted)
            {
                switch (operation)
                {
                    ...
                    case DocOperation.submit:
                        break;
                    case DocOperation.pass:
                        nextStatus = DocStatus.passed;
                        break;
                    case DocOperation.reject:
                        nextStatus = DocStatus.rejected;
                        break;
                    default:
                        break;
                }
            ...
            else if (currentStatus == DocStatus.modified)
            {
                switch (operation)
                {
                    case DocOperation.save:
